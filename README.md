...
**lidis** (Lightweight Dictionary Server) 是一个基于 C 语言开发的轻量级 Key-Value 存储系统。
...
`lidis` 的主从全量同步使用 `RDMA`，需要通过 `rdma_rxe` 模拟一块 `RDMA` 网卡：
...
```bash
git clone --recurse-submodules https://github.com/yansss626/lidis.git
```
...
`lidis`  通过 `conf/kvstore.conf` 配置运行参数，启动时自动读取。
...
| `Port` | `lidis`   服务监听端口，客户端通过此端口连接 | `2000` |
...
| 配置项 | 说明 | 角色 |
|-------|------|-----|
| `Master_ip` | 主端 `lidis`   的 IP 地址 | 从端配置 |
| `Master_port` | 主端 `lidis`   的服务端口 | 从端配置 |
...
`lidis`  当前支持两种客户端通信协议：自定义的 **`KVSP` 协议** 和兼容 `redis` 的 **`RESP` 协议**。服务端会在客户端连接建立后，根据首个请求报文的起始内容自动识别协议类型，并在该连接后续通信中使用对应的解析方式。

注：目前只兼容 `RESP` 协议对 `lidis`  数据的增（`SET`）、删（`DEL`）、查（`GET`）、改（`MOD`）四种命令。可使用 `redis-cli` 工具快速验证。
...
| 协议   | 说明                                                         |
| ------ | ------------------------------------------------------------ |
| `KVSP` | `lidis`   自定义协议，使用 `#<body_length>\r\n^<tok_len>&<tok>...\r\n` 格式描述一条完整命令 |
...
`lidis`   同时兼容 `redis RESP` 协议中的数组和批量字符串格式，因此可以使用 `hiredis` 等 `redis` 客户端发送请求。
...
需要注意的是，`lidis`   当前主要兼容 `redis RESP` 的命令编码格式，并不表示完整实现 `redis` 的所有命令语义。

### 3.5 响应格式

 `lidis` 的响应格式保持 `RESP-like` 风格，主要包括：
...
数据落盘用 `io_uring` 实现，加载持久化数据用 `mmap`。

### 5.2 全量持久化 （SAVE）
提供 `SAVE` 指令，用于将当前内存中的全量数据持久化到 `.rdb` 文件中。
...
`testcase_kvsp/` 目录提供了一系列基于 `KVSP` 协议的测试客户端，用于验证 `lidis` 各项功能。
...
```
    测试 lidis 基本功能 SET/GET/MODE/DEL/EXIST 是否正常。

### SET 类写入指令测试
```bash
./set <ip> <port>
```
    模拟客户端发送 SET/RSET/LSET/HSET 命令，若 lidis 成功响应，打印结果。

### GET 类读取指令测试
```bash
./get <ip> <port>
```
    模拟客户端发送 GET/RGET/LGET/HGET 命令，若 lidis 成功响应，打印结果。

### DEL 类删除指令测试
```bash
./del <ip> <port>
```
    模拟客户端发送 DEL/RDEL/LDEL/HDEL 命令，若 lidis 成功响应，打印结果。

### 数据持久化与 SAVE 指令测试
```bash
./set_save <ip> <port>
```
    模拟客户端发送 GET/RGET/LGET/HGET 命令后，再发送 SAVE 命令 若 lidis 成功响应，打印结果。

### 大 Key-Value 批量写入测试
```bash
./set_blog <ip> <port>
```
    模拟客户端发送大 key 大 value 键值对，打印接收结果。

### 大 Key-Value 读取验证测试
```bash
./get_blog <ip> <port>
```
    模拟客户端发送 GET 大 key，打印 set_blog.c 插入的大 value。
...
`Linux` 内核版本：`ubuntu 22.04.5  6.8.0-107-generic`
...
| 测试条件 \ 测试对象 | `redis` | `lidis` |
| :--- | :---: |  :---: |
| **关闭 `AOF`（写性能）** | 3097 | 3272 |
| **关闭 `AOF`（读性能）** | 3188 | 3244 |
| **开启 `AOF`（写性能）** | 2953 | 3208 |
| **开启 `AOF`（读性能）** | 3004 | 3260 |
...
| :--- | :---: |
| **关闭主从同步** | 3334 |
| **开启主从同步：探测 `TCP` 的 `recv`** | 2036 |
| **开启主从同步：网络 `send` 转发** | 2700 |
