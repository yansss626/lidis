...
#define MAX_CACHE_SIZE      (128 * 1024 * 1024) // 128 MB
...
int kvs_agent_init() {
    ...
    ret = kvs_agent_cache_create(&(global_agent_cache), MAX_CACHE_SIZE);
    if (ret != 0) {
    ...
    ret = pthread_create(&agent_tid, NULL, agent, NULL);
    if (ret != 0) {
