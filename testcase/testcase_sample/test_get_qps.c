...
void array_testcase(int connfd, int count) {
	...
	for (i = 0;i < count;i ++) {
		...
		char cmd[128] = {0};
		snprintf(cmd, 128, "GET Teacher%d King%d", i, i);
        char reply[128] = {0};
        snprintf(reply, 128, "King%d\r\n", i);
		testcase(connfd, cmd, reply, cmd);
	}
...
void hash_testcase(int connfd, int count) {
	...
	for (i = 0;i < count;i ++) {
		...
		char cmd[128] = {0};
		snprintf(cmd, 128, "HGET Teacher%d King%d", i, i);
        char reply[128] = {0};
        snprintf(reply, 128, "King%d\r\n", i);
		testcase(connfd, cmd, reply, cmd);
	}
...
void skiplist_testcase(int connfd, int count) {
	...
	for (i = 0;i < count;i ++) {
		...
		char cmd[128] = {0};
		snprintf(cmd, 128, "LGET Teacher%d King%d", i, i);
        char reply[128] = {0};
        snprintf(reply, 128, "King%d\r\n", i);    
		testcase(connfd, cmd, reply, cmd);
	}
